...
CSCAN_MODELS_DIR = os.path.join(_BACKEND_DIR, "storage", "cscan", "models")
DEFAULT_SRC_MODEL = os.path.join(
    _BACKEND_DIR, "storage", "cscan", "runs", "cscan_v1", "weights", "best.pt"
)
...
def _ensure_default_model() -> str:
    """首启：把训练产物 best.pt 拷进 backend/storage/cscan/models（已 gitignore，不入库），
...
def list_cscan_models() -> list:
    """backend/storage/cscan/models 下的模型清单，按修改时间倒序。"""
