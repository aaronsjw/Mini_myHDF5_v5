...
- 训练完把 best.pt 拷进 backend/storage/cscan/models/<name>.pt + 写 .json meta，即被 /cscan/models 与 analyze 收编。
"""
...
DS_DIR = os.path.join(_REPO_DIR, "dataset", "cscan_dataset")   # 纯数据 + 数据集定义
RUNS_DIR = os.path.join(_BACKEND_DIR, "storage", "cscan", "runs")             # 训练/评估产物（原 dataset/cscan_dataset/runs）
...
MODELS_DIR = os.path.join(_BACKEND_DIR, "storage", "cscan", "models")
...
# ════════════ 训练集划分（进程内复刻 backend/cscan/tools/split_dataset.py 核心）════════════
