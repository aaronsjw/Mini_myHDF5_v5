...
import { Card, Button, Tag, Select, Space, message } from 'antd'
...
export default function InspectPanel({
    ...
    const [defectType, setDefectType] = useState('OK')

    useEffect(() => {
        if (editableJson && editableJson.defectType !== undefined) {
            setDefectType(editableJson.defectType)
        }
    }, [editableJson])
    ...
    return (
        <Card
            title="Dataset Info"
            extra={<Button type="primary" onClick={saveAsJson}>Save As</Button>}
        >
            ...
            <div style={{ marginTop: 15, height: 'calc(100vh - 220px)', overflow: 'auto' }}>
                {editableJson?.defectType !== undefined && (
                    <Card size="small" title="Defect Type Label" style={{ marginBottom: 16 }}>
                        <Space>
                            <Select
                                value={defectType}
                                style={{ width: 150 }}
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    link.href = 'http://127.0.0.1:8000/download_nde'
                                    link.download = 'modified.nde'
                                    link.click()
                                }}
                            >
                                Save & Download
