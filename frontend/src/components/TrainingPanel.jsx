...
export default function TrainingPanel({ active }) {
    // 数据集预览
    ...
    // 模型配置
    const [modelType, setModelType] = useState('random_forest')   // RF or DL
    const [testSize, setTestSize] = useState(0.2)                 // 验证集比例
    const [nEstimators, setNEstimators] = useState(100)           // RF：树数量
    const [maxDepth, setMaxDepth] = useState(null)                // RF：深度
    const [epochs, setEpochs] = useState(20)                      // DL：epoch
    const [batchSize, setBatchSize] = useState(16)                // DL：批次
    const [learningRate, setLearningRate] = useState(0.001)       // DL：学习率
    ...
    //    preview_dataset返回：
    //        total_files → 总文件数
    //        n_features_total → 特征维度（现在显示 405 维）
    //        by_defect → 缺陷分布饼图
    //        by_material → 材料分布饼图
    useEffect(() => {
    ...
    const handleDeleteModel = (name) => {
        axios.delete(`http://127.0.0.1:8000/train/models/${name}`)
            .then(() => { message.success('已删除'); loadModels() })
    ...
    // 已训练模型列表列
    const modelColumns = [
        { title: '模型名称', dataIndex: 'model_name', key: 'model_name', ellipsis: true },
        { title: '类型', dataIndex: 'model_type', key: 'model_type', width: 110, render: v => v === 'random_forest' ? <Tag color="blue">随机森林</Tag> : <Tag color="purple">深度学习</Tag> },
        { title: '准确率', dataIndex: 'accuracy', key: 'accuracy', width: 90, render: v => (v * 100).toFixed(1) + '%' },
        { title: '保存时间', dataIndex: 'saved_at', key: 'saved_at', width: 160 },
        { title: '操作', key: 'action', width: 80, render: (_, r) => <Button size="small" danger onClick={() => handleDeleteModel(r.model_name)}>删除</Button> },
    ]
        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 12, overflow: 'auto', paddingRight: 4 }}>
            {previewLoading ? (
                ...
                <>
                    ...
                    <Row gutter={12} align="stretch">
                        <Col span={8}>
                            <Card size="small" title="超声AScan数据集概览" style={{ height: '100%' }}>
                                ...
                                <div style={{ marginBottom: 16 }}>
                                    <span style={{ color: 'rgba(0, 0, 0, 0.45)' }}>特征维度：</span>
                                    <strong>{preview.n_features_total || preview.n_features}</strong> 维（含元数据）
                                </div>
                                <div style={{ marginBottom: 16 }}>
                                    <span style={{ color: 'rgba(0, 0, 0, 0.45)' }}>缺陷类别：</span>
                                    <strong>{Object.keys(preview.by_defect || {}).length}</strong> 类
                                </div>
                                <div>
                                    <span style={{ color: 'rgba(0, 0, 0, 0.45)' }}>材料体系：</span>
                                    <strong>{Object.keys(preview.by_material || {}).length}</strong> 种
                                </div>
                            </Card>
                        </Col>
                        <Col span={8}>
                    ...
                    {/* ═══ 已训练模型列表 ═══ */}
                    <Card size="small" title="已训练模型" extra={<Button size="small" onClick={loadModels}>刷新</Button>}>
                        <Table
